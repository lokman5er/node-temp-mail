...
function buildJson(callback, label, currentJson, addressJson, currentIndex, finalIndex) {
  ...
  request({
    url: "https://www.1secmail.com/api/v1/?action=readMessage&login="+label+"&domain=kzccv.com&id="+addressJson[currentIndex].id,
    json: true
...
class TempMail {
  ...
  fetchEmails(callback){
    ...
    request({
      url: "https://www.1secmail.com/api/v1/?action=getMessages&login="+mailingAddressLabel+"&domain=kzccv.com",
      json: true
    }, function (error, response, body) {
      if (error || response.statusCode !== 200) {
        return callback(error || {statusCode: response.statusCode});
        ...
        response = {};
        response.address = mailingAddressLabel + "@kzccv.com";
  ...
  getAddress(){
    var response = {};
    response.address = this.mailingAddressLabel + "@kzccv.com";
    return response;
  }
}
